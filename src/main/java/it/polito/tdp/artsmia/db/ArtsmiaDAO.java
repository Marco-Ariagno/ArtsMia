...
public class ArtsmiaDAO {
		...
		try {
			...
			while (res.next()) {
				...
				if (!idMap.containsKey(res.getInt("object_id"))) {
					...
					//E' passata per riferimento, non serve return
				}
			}
			conn.close();
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		String sql="SELECT COUNT(*) AS peso FROM exhibition_objects AS eo1, exhibition_objects AS eo2 " + 
	...
	public List<Adiacenza> getAdiacenza(){
		final String sql="SELECT eo1.object_id as obj1, eo2.object_id as obj2, COUNT(*) AS peso FROM exhibition_objects AS eo1, exhibition_objects AS eo2 " + 
